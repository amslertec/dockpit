...
<div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center z-[1000] p-0 sm:p-4 modal-backdrop-enter" onclick={(e) => { if (e.target === e.currentTarget) onclose(); }}>
	<div class="border border-[var(--border)] rounded-t-[var(--radius-xl)] sm:rounded-[var(--radius-xl)] w-full {wide ? 'sm:max-w-3xl' : 'sm:max-w-lg'} max-h-[90vh] sm:max-h-[85vh] overflow-y-auto shadow-[var(--shadow-lg)] modal-content-enter modal-glass">
		<div class="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
			...
			<button class="w-8 h-8 flex items-center justify-center rounded-[var(--radius-md)] border border-[var(--border)] text-[var(--text-secondary)] hover:text-[var(--red)] hover:border-[var(--red)]/40 hover:bg-[var(--red)]/8 transition-all duration-200" onclick={onclose}>
