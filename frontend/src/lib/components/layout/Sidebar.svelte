<script lang="ts">
	...
	const navItems = [
		...
		{ href: '/dependencies', key: 'nav.dependencies', icon: 'dep', perm: 'page.containers' },
	];
				...
				{#each navItems.filter(item => $canSeePage(item.perm)) as item}
					<a href={item.href} class="nav-item {isActive(item.href) ? 'nav-active' : ''}" onclick={onclose}>
						...
						{:else if item.icon === 'heart'}
							<svg class="w-[18px] h-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>
						...
						{:else if item.icon === 'dep'}
							<svg class="w-[18px] h-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="6" cy="6" r="3"/><circle cx="18" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="18" r="3"/><path d="M9 6h6M6 9v6M18 9v6M9 18h6"/></svg>
						{/if}
						{$t(item.key)}
					</a>
				{/each}
