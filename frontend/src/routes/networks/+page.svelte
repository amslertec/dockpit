<script lang="ts">
	...
	$effect(() => {
		if (!$canSeePage('page.networks')) goto('/profile');
	});
...
<div class="bg-card border border-theme rounded-lg overflow-hidden">
	...
	{#if someSelected && $canDoAction('action.network_delete')}
				...
				<tbody>
					{#each paged as n}
						<tr class="border-b border-theme last:border-0 hover:bg-hover transition {!n.in_use ? 'opacity-70' : ''} {selected.has(n.id) ? 'bg-accent-light' : ''}">
							{#if $canDoAction('action.network_delete')}<td class="w-10 px-4 py-3">
							...
							<td class="px-4 py-3">
								...
								{:else}
									<button class="w-7 h-7 flex items-center justify-center rounded-[var(--radius-sm)] border border-theme text-[var(--red)] hover:border-[var(--red)]/40 hover:bg-[var(--red)]/8 transition" title={$t('common.delete')} onclick={() => remove(n.id, n.name)}>
										<svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>
									</button>
								{/if}
							</td>
